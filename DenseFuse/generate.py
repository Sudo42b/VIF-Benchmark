...
def writexls(save_name, num, time):
    if not os.path.exists(save_name):
        print('保存测试时间！')
        # 判断文件是否存在，若不存在则新建一个xls文件保存数据
        workbook = xlwt.Workbook(encoding='utf-8')  # 新建工作簿
        sheet1 = workbook.add_sheet('Time')  # 新建sheet
        sheet1.write(0, 4, "DenseFuse")
        workbook.save(save_name)  # 保存
    workbook1 = xlrd.open_workbook(save_name)
    workbook = copy(workbook1)

    sheet1 = workbook.get_sheet('Time')  # 新建sheet
    sheet1.write(0, 4, "DenseFuse")
    sheet1.write(num, 4, time)

    workbook.save(save_name)  # 保存


def generate(infrared_path, visible_path, model_path, model_pre_path, ssim_weight, index, IS_VIDEO, IS_RGB,
             type='addition', output_path=None, name=None):
    if IS_VIDEO:
        ...
        if IS_RGB:
            ...
            if type == 'addition':
                print('addition')
                temp_time = _handler(infrared_path, visible_path, model_path, model_pre_path, output_path=output_path)
                return temp_time
            elif type == 'l1':
                print('l1')
                _handler_l1(infrared_path, visible_path, model_path, model_pre_path, ssim_weight, index,
                            output_path=output_path)


def _handler(ir_path, vis_path, model_path, model_pre_path, output_path=None):
    ...
    with tf.Graph().as_default() as graph, tf.Session() as sess:
        ...
        # print(num)
        # save_name = 'test_time.xls'
        # writexls(save_name, item, '%.6f' % (end_time - start_time))
...
def _handler_rgb(ir_path, vis_path, model_path, model_pre_path, ssim_weight, index, output_path=None, name=None):
    # ir_img = get_train_images(ir_path, flag=False)
    # vis_img = get_train_images(vis_path, flag=False)
    ...
    with tf.Graph().as_default(), tf.Session() as sess:
        ...
        save_images(ir_path, output, output_path, prefix=name)
...
def _handler_rgb_l1(ir_path, vis_path, model_path, model_pre_path, ssim_weight, index, output_path=None):
    ...
    ir_img = get_test_image_rgb(ir_path, flag=False)
    vis_img = get_test_image_rgb(vis_path, flag=False)
    dimension = vi_img.shape

    ir_img = ir_img.reshape([1, dimension[0], dimension[1], 1])
    vis_img = vis_img.reshape([1, dimension[0], dimension[1], dimension[2]])
